...
export function useReceiptUpload({
	...
	const { draft, isHydrated, saveDraft, clearDraft } = useDraft<ReceiptDraft>("receipt", receiptId);
	...
	useEffect(() => {
		if (isHydrated && draft) {
			...
			if (draft.tempUrl && draft.fileName && !selectedFile) {
				console.log("[useReceiptUpload] Fetching file from blob URL:", draft.tempUrl);
				fetch(draft.tempUrl)
					.then(res => res.blob())
					.then(blob => {
						...
						console.log("[useReceiptUpload] Restored file from blob:", file.name);
					})
					.catch(err => {
						console.error("[useReceiptUpload] Failed to fetch file from blob:", err);
					});
			}
		} else {
			console.log("[useReceiptUpload] No draft to restore. isHydrated:", isHydrated, "draft:", draft);
		}
	...
	const analyzeReceipt = useCallback(async (file: File) => {
		setIsAnalyzing(true);
		const toastId = toast.loading(t("treasury.receipts.analyzing", "Analyzing receipt..."));
		...
		try {
			...
			if (result.success) {
				...
				toast.success(t("treasury.receipts.analysis_complete", "Receipt analyzed successfully"), {
					id: toastId,
				});
			} else {
			...
			const errorMessage = error instanceof Error && error.name === 'AbortError'
				? t("treasury.receipts.analysis_timeout", "Analysis timed out. The file may be too large or the server is slow.")
				: t("treasury.receipts.analysis_failed", "Failed to analyze receipt");
			toast.error(errorMessage, { id: toastId });
		} finally {
			setIsAnalyzing(false);
		}
	}, [tempUrl, tempPathname, onNameSuggestion, onDescriptionSuggestion, saveDraft, t]);
	...
	const handleFileChange = useCallback(async (file: File | null) => {
		...
		setIsUploading(true);
		const uploadToastId = toast.loading(t("treasury.receipts.uploading", "Uploading file..."));
		...
		try {
			...
			if (uploadResult.success) {
				...
				console.log("[useReceiptUpload] Saving draft after upload:", draftData);
				saveDraft(draftData);

				toast.success(t("treasury.receipts.upload_complete", "File uploaded successfully"), {
					id: uploadToastId,
				});
			...
			const errorMessage = error instanceof Error && error.name === 'AbortError'
				? t("treasury.receipts.upload_timeout", "Upload timed out. The file may be too large.")
