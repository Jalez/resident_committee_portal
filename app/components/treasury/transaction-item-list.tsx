...
export function TransactionItemList({
	...
	const updateQuantity = (itemId: string, qty: number) => {
		onItemsChange(
			items.map((i) => (i.itemId === itemId ? { ...i, quantity: qty } : i)),
		);
	};
	...
	const handlePickerSelectionChange = (newIds: string[]) => {
		...
		for (const id of newIds) {
			...
			if (existingItem) {
				nextItems.push(existingItem); // Keep existing with quantity
			} else {
				// Find from available
			...
			{items.length > 0 && (
				<div className="space-y-2 border-t border-gray-100 dark:border-gray-700 pt-3">
					{items.map((item) => (
						<div
							key={item.itemId}
							className="flex justify-between items-center text-sm bg-gray-50 dark:bg-gray-900/50 p-2 rounded-lg gap-2"
						>
