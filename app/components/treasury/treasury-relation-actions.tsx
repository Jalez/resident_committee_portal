...
import {
	type EntityType,
	encodeSourceContext,
} from "~/lib/linking/source-context";
...
type TreasuryRelationActionsProps = {
	label: string;
...
function buildAddUrl(
	...
	const context = encodeSourceContext({
		type: source.type,
		id: source.id,
		name: source.name,
	});
...
export function TreasuryRelationActions({
	label,
	items,
	withSeparator = false,
	mode = "view",
		...
		<>
			...
			<div className="space-y-2">
				...
				<div className="flex flex-wrap gap-2 items-center">
					...
					{mode === "edit" && (maxItems === undefined || items.length < maxItems) && (
						<>
							{!isExpanded ? (
								<Button
									type="button"
												...
												{linkableItems.length > 0 ? (
													linkableItems.map((item) => (
														<ColoredStatusLinkBadge
															...
															onCheck={() => setSelectedLinkId(selectedLinkId === item.id ? null : item.id)}
															checked={selectedLinkId === item.id}
															variantMap={item.variantMap}
															className={selectedLinkId === item.id ? "border-2 border-dashed border-current" : ""}
														/>
													))
												) : (
