...
export function Field({
    ...
    const canTranslate = !!(localModel && user?.localOllamaEnabled && (type === "text" || type === "textarea") && sourceLanguage && targetLanguage);
    ...
    const handleQuickTranslate = async () => {
        ...
        const sourceName = isSecondary ? name.replace("Secondary", "") : `${name}Secondary`;

        const sourceInput = document.getElementById(sourceName) as HTMLInputElement | HTMLTextAreaElement | null;
        const sourceValue = sourceInput?.value;

        if (!sourceValue) {
            toast.error(t("field.translation.no_source", { defaultValue: "Source field is empty" }));
            return;
        }
        ...
        try {
            ...
            onChange?.(result);
            toast.success(t("field.translation.success", { defaultValue: "Translated!" }));
        } catch (err) {
            console.error(err);
            toast.error(t("field.translation.error", { defaultValue: "Translation failed" }));
        } finally {
        ...
        return (
            <div className={cn("flex items-center gap-3 space-y-0", className)}>
                ...
                {label && (
                    <Label
                        htmlFor={name}
                        className={cn("cursor-pointer font-normal", error ? "text-destructive" : "")}
                    >
                        {label}
                ...
                <div className="flex items-center justify-between">
                    ...
                    {canTranslate && (isHovered || isTranslating) && (
                        <button
                            ...
                            title={t("field.translate_hint", { defaultValue: "Translate from other language" })}
                        >
                            {isTranslating ? (
            ...
            <div className="relative">
                {type === "select" && options ? (
                    <Select
                        ...
                        required={required}
                    >
                        <SelectTrigger id={name} className={cn(error && "border-destructive", valueClassName)}>
                    ...
                    <div className="relative">
                        <Input
                            ...
                            disabled={disabled}
                            className={cn("pr-8", error && "border-destructive", valueClassName)}
                        />
