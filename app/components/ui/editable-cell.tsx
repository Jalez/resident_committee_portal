...
export function EditableCell({ value, onSave, disabled = false, items, type = "text", min, step }: EditableCellProps) {
    ...
    useEffect(() => {
        ...
        const handleClickOutside = (event: MouseEvent) => {
            ...
            const isInPopover = (target as Element).closest('[data-radix-popper-content-wrapper]') ||
    ...
    }, [isEditing, items, editValue, value]); // Dependencies for handleSave closure
    ...
    if (isEditing) {
        if (items) {
            return (
                <div ref={containerRef} onClick={(e) => e.stopPropagation()} className="min-w-[150px]">
                    <SmartCombobox
                        items={items}
                        value={editValue}
