...
import type { EnvStatus, EnvCategory, EnvVariable } from "~/lib/env-config.server";
        ...
        <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
            <div
                className={cn(
                    ...
                    percentage === 100 ? "bg-green-500" : percentage > 50 ? "bg-yellow-500" : "bg-primary"
                )}
...
function CategoryCard({ category, defaultExpanded = false }: { category: EnvCategory; defaultExpanded?: boolean }) {
    const [expanded, setExpanded] = useState(defaultExpanded);
    const hasMissingRequired = category.variables.some(v => v.required && !v.isSet);
    ...
    return (
        <Card className={cn(
            ...
            >
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="text-right">
                            <span className={cn(
                                "text-sm font-medium",
                                category.isFullyConfigured ? "text-green-600 dark:text-green-400" : ""
                            )}>
                ...
                <div className="mt-3">
                    <ProgressBar value={category.configuredCount} max={category.totalCount} />
                </div>
            </CardHeader>
...
export function SetupGuide({ envStatus }: SetupGuideProps) {
    const percentage = envStatus.totalVariables > 0
            ...
            <div className="max-w-3xl mx-auto space-y-6">
                ...
                <Card className="bg-gradient-to-br from-card to-muted/30">
                    <CardContent className="pt-6">
                        ...
                        <ProgressBar value={envStatus.totalConfigured} max={envStatus.totalVariables} />
                        <p className="text-xs text-muted-foreground mt-2">
                            {envStatus.totalConfigured} of {envStatus.totalVariables} variables configured
                        </p>
                    </CardContent>
                </Card>
                ...
                {envStatus.missingCritical.length > 0 && (
                    <Alert variant="destructive">
                        ...
                        <AlertTitle>Missing Required Variables / Puuttuvat pakolliset muuttujat</AlertTitle>
                        <AlertDescription>
                            The following variables are required to start the app:
                            <div className="mt-2 flex flex-wrap gap-2">
                                {envStatus.missingCritical.map(name => (
                                    <code key={name} className="bg-destructive/20 px-2 py-0.5 rounded text-xs">
                                        {name}
                                    </code>
                                ))}
                            </div>
                ...
                {envStatus.canStart && (
                    <Alert className="border-green-500/30 bg-green-500/5">
                        ...
                        <AlertDescription>
                            Core configuration is complete. You can start the app with <code className="bg-muted px-1.5 py-0.5 rounded">bun dev</code>.
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <p className="text-sm text-muted-foreground">
                                2. Edit <code className="bg-muted px-1 rounded">.env</code> and fill in your values
                            </p>
                        </div>
                        <div className="space-y-2">
                            <p className="text-sm text-muted-foreground">
                ...
                <div className="space-y-4">
                    ...
                    {envStatus.categories.map((category, index) => (
                        <CategoryCard
                            ...
                            defaultExpanded={index === 0 || category.variables.some(v => v.required && !v.isSet)}
                        />
                    ))}
                </div>
                ...
                <div className="text-center py-8 text-sm text-muted-foreground">
                    ...
                    <p className="mt-1 text-xs">
                        Refresh this page after updating your <code className="bg-muted px-1 rounded">.env</code> file.
                    </p>
                </div>
            </div>
        </div>
