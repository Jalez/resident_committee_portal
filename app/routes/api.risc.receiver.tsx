...
export async function action({ request }: ActionFunctionArgs) {
    ...
    try {
        ...
        const clientIdsStr = process.env.GOOGLE_CLIENT_IDS || process.env.GOOGLE_OAUTH_CLIENT_ID || "";
        const clientIds = clientIdsStr.split(",").map(id => id.trim()).filter(Boolean);

        if (clientIds.length === 0) {
            console.error("❌ Neither GOOGLE_CLIENT_IDS nor GOOGLE_OAUTH_CLIENT_ID is configured in .env");
        ...
        console.log("🔒 RISC Security Event Received:", JSON.stringify(verified, null, 2));
        ...
        for (const [eventType, details] of Object.entries(events)) {
            ...
            if (eventType === "https://schemas.openid.net/secevent/risc/event-type/verification") {
                console.log("✅ RISC verification successful. State:", (details as any).state);
            }
        ...
        console.error("❌ RISC Token Validation Failed:", error.message);
        return new Response(JSON.stringify({ error: "Invalid token", message: error.message }), {
            status: 400,
            headers: { "Content-Type": "application/json" }
        });
    }
