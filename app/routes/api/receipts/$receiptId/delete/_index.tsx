...
export async function loader({
	...
	console.log(
		"[api.receipts.$receiptId.delete] Loader hit (should not happen for DELETE)",
	);
...
export async function action({ request, params }: ActionFunctionArgs) {
	...
	console.log(
		`[api.receipts.$receiptId.delete] Action hit for ID: ${receiptId}, Method: ${request.method}`,
	);

	if (request.method !== "DELETE" && request.method !== "POST") {
		console.log(
			`[api.receipts.$receiptId.delete] Rejected method: ${request.method}`,
		);
		return new Response(JSON.stringify({ error: "Method not allowed" }), {
			status: 405,
