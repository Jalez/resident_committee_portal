...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const result = {
    success: true,
    deletedCount: 0,
    deleted: [] as Array<{ type: RelationshipEntityType; id: string; name?: string }>,
    errors: [] as string[],
  };
  ...
  try {
    ...
    for (const entityType of draftableTypes) {
      try {
        ...
        const orphanedIds = await db.getOrphanedDrafts(entityType, olderThanMinutes);
        
        if (orphanedIds.length === 0) {
          continue;
        }

        console.log(`[DraftCleanup] Found ${orphanedIds.length} orphaned ${entityType} drafts`);
        ...
        for (const id of orphanedIds) {
          try {
            ...
            console.error(`[DraftCleanup] Failed to delete files for ${entityType} ${id}:`, error);
            // Continue with entity deletion even if file deletion fails
          }
        }

        // Delete the orphaned entities
        const deletedCount = await db.bulkDeleteDraftEntities(entityType, orphanedIds);
...
async function deleteAssociatedFiles(
  ...
  if (entityType === "receipt") {
    ...
    if (minute?.fileKey) {
      try {
        const { getMinuteStorage } = await import("~/lib/minutes/storage.server");
