...
import type { Route } from "./+types/admin.storage.minutes";
...
export async function loader({ request }: Route.LoaderArgs) {
    ...
    const items = allFiles.map((file) => {
        ...
        return {
            ...
            minuteDate: minute?.date ? new Date(minute.date).toISOString().split('T')[0] : null,
...
export async function action({ request }: Route.ActionArgs) {
    ...
    await requireAnyPermission(request, ["admin:storage:write", "minutes:delete"], getDatabase);
...
export default function AdminStorageMinutes() {
    ...
    const submit = useRevalidator(); // actually we can use useSubmit or just fetch. 
    ...
    const doDelete = useCallback(
        async (pathname: string) => {
            ...
            try {
                ...
                const res = await fetch(window.location.href, { // POST to self
                    method: "POST",
                    body: formData,
                });
                if (!res.ok) {
    ...
    return (
        <PageWrapper>
            <ConfirmDialog
                ...
                description={t("admin.storage.minutes.delete_confirm", "Are you sure you want to delete this file? This cannot be undone.")}
            ...
            >
                <p className="text-muted-foreground mb-6 -mt-6">
                    {t("admin.storage.minutes.description", "Manage uploaded minute files. Linked files correspond to existing minute entries.")}
                </p>
                                        ...
                                        {item.isLinked ? (
                                            ...
                                            <>
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => setDeleteConfirmPathname(item.pathname)}
                                                >
