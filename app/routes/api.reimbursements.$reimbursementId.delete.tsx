...
export async function action({ request, params }: ActionFunctionArgs) {
    ...
    if (!reimbursementId) {
        return new Response(JSON.stringify({ error: "Reimbursement ID is required" }), {
            status: 400,
            headers: { "Content-Type": "application/json" },
        });
    }
    ...
    if (purchase.emailSent && purchase.status !== "rejected") {
        return new Response(
            JSON.stringify({
                error: "Cannot delete a reimbursement request that has already been sent. Reject it first if needed.",
            }),
            {
                status: 400,
                headers: { "Content-Type": "application/json" },
