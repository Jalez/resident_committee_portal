...
import {
	NewsAiSettings,
} from "~/components/settings/news-ai-settings";
import {
	handleNewsAiSettingsAction,
} from "~/components/settings/news-ai-settings.server";
...
export default function SettingsNews({ loaderData }: Route.ComponentProps) {
	...
	return (
		<PageWrapper>
			<PageHeader title={t("settings.news.title", { lng: systemLanguages.primary })} />
			<NewsAiSettings
				newsModel={newsModel}
				hasApiKey={hasApiKey}
				models={models}
