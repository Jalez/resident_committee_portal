...
export async function action({ request, params }: ActionFunctionArgs) {
    ...
    if (!reimbursementId) throw new Response("Reimbursement ID required", { status: 400 });
    ...
    // Save relationships
    await saveRelationshipChanges(db, "reimbursement", reimbursementId, formData, user?.userId || null);

    // Auto-publish draft
    if (purchase.status === "draft") {
        const newStatus = getDraftAutoPublishStatus("reimbursement", purchase.status, {
            description: description || purchase.description,
            amount: amount || purchase.amount,
            purchaserName,
            bankAccount,
    ...
    return redirect(`/treasury/reimbursements?year=${purchase.year}&success=Reimbursement updated`);
}
