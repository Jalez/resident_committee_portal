...
import { analyzeRelationshipContext, type AIEnrichmentResult } from "~/lib/ai/relationship-analyzer.server";
import type { RelationshipContext } from "~/lib/linking/relationship-context.server";
import type { Route } from "./+types/api.entities.smart-autofill";
...
export async function action({ request }: Route.ActionArgs) {
	await requireAnyPermission(
		request,
		["treasury:transactions:update", "treasury:budgets:update", "treasury:reimbursements:update", "inventory:write"],
		getDatabase,
	);
	const db = getDatabase();

	const formData = await request.formData();
	const entityType = formData.get("entityType") as RelationshipEntityType | null;
