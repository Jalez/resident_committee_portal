...
import type { Route } from "./+types/api.relationship.analyze";
...
export async function action({ request }: Route.ActionArgs) {
	...
	if (!entityType || !entityId) {
		return data({ success: false, error: "Missing entityType or entityId" }, { status: 400 });
	}
	...
	try {
		...
		if (!analyzer) {
			return data(
				{ success: false, error: `No analyzer available for entity type: ${entityType}` },
				{ status: 400 }
			);
		}
		...
		for (const suggestion of analysisResult.suggestions) {
			try {
				const draftEntity = await createDraftEntity(suggestion.entityType, suggestion.data, db);
				if (draftEntity) {
				...
				console.error(`[AnalyzeAPI] Failed to create ${suggestion.entityType}:`, error);
				errors.push(`Failed to create ${suggestion.entityType}: ${error instanceof Error ? error.message : String(error)}`);
			}
		}
...
async function checkPermissions(
	...
	if (permission && createdBy !== null) {
		await requirePermissionOrSelf(request, permission, `${permission}-self`, createdBy, getDatabase);
	}
}
...
async function createDraftEntity(
	...
	switch (entityType) {
		case "transaction":
			return db.createTransaction(data as Parameters<typeof db.createTransaction>[0]);
		case "inventory":
			return db.createInventoryItem(data as Parameters<typeof db.createInventoryItem>[0]);
		case "reimbursement":
		...
		case "budget":
			return db.createFundBudget(data as Parameters<typeof db.createFundBudget>[0]);
		case "receipt":
