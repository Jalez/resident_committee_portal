...
import { getDatabase, type MailDraft, type CommitteeMailMessage } from "~/db";
...
import { addReplyPrefix, addForwardPrefix } from "~/lib/mail-utils";
import {
...
import type { Route } from "./+types/mail.compose";
...
export async function action({ request }: Route.ActionArgs) {
	...
	if (intent === "createDraft") {
		const draftType =
			(formData.get("draftType") as ComposeMode) || "new";
	...
	if (intent === "send") {
		...
		const forwardFromMessageId =
			(formData.get("forwardFromMessageId") as string) || null;
		const composeMode =
			(formData.get("composeMode") as ComposeMode) || "new";
...
export default function MailCompose({ loaderData }: Route.ComponentProps) {
	...
	const getInitialTo = (): RecipientEntry[] => {
		...
		if (composeMode === "replyAll") {
			...
			for (const r of toList) {
				if (
					r.email.toLowerCase() !==
					committeeFromEmail.toLowerCase()
				) {
					recipients.push(r);
				}
			}
	...
	// Auto-create draft on first mount if none exists
	useEffect(() => {
		if (draftId || createFetcher.data?.draftId) return;
		if (createFetcher.state === "idle" && !createFetcher.data) {
			const fd = new FormData();
			fd.set("_action", "createDraft");
			fd.set("draftType", composeMode);
			if (
				originalMessage &&
				(composeMode === "reply" || composeMode === "replyAll")
			) {
				fd.set("replyToMessageId", originalMessage.id);
			}
			if (originalMessage && composeMode === "forward") {
				fd.set("forwardFromMessageId", originalMessage.id);
			}
			createFetcherRef.current.submit(fd, { method: "post" });
		}
	}, [
		draftId,
		createFetcher.state,
	...
	// After draft created, store the ID
	useEffect(() => {
		const id = createFetcher.data?.draftId;
		if (id) {
			setCurrentDraftId(id);
		}
	}, [createFetcher.data?.draftId]);
	...
	useEffect(() => {
		if (
			...
			setLastSavedAt(
				new Date(updateFetcher.data.updatedAt as string),
			);
		}
		...
		(entries: { email: string; name?: string }[]) => {
			setToRecipients((prev) => {
				const byEmail = new Set(
					prev.map((r) => r.email.toLowerCase()),
				);
				const next = [...prev];
