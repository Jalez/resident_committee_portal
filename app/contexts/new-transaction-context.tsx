...
const NewTransactionContext = createContext<NewTransactionContextType | null>(null);
...
export function useInitializeTransactionItems() {
    ...
    return useCallback((selections: { itemId: string; name: string; quantity: number; unitValue: number }[]) => {
        setItems(selections.map(sel => ({
            itemId: sel.itemId,
            name: sel.name,
