...
import { getInfoReelNavItems } from "~/lib/nav-config";
...
const FILL_PHASE = 0.7; // 70% - progress bar fills
const HOLD_PHASE = 0.2; // 20% - stays at 100%
const FADE_PHASE = 0.1; // 10% - fades out (and next fades in)
...
export function useLocalReel<T>({
	items,
}: UseLocalReelOptions<T>): UseLocalReelResult<T> {
	...
	const itemDuration =
		items.length > 0
			? currentRouteDuration / items.length
...
export function InfoReelProvider({ children }: { children: ReactNode }) {
	...
	const getCurrentRouteIndex = useCallback(() => {
		return reelRoutes.findIndex(
			(r: RouteConfig) => r.path === location.pathname,
		);
	}, [location.pathname, reelRoutes]);
	...
	useEffect(() => {
		...
		const intervalId = setInterval(() => {
			...
			const remaining = Math.max(
				0,
				100 - (elapsed / currentRouteDuration) * 100,
			);
	...
	}, [isInfoReel, navigateToNextRoute, currentRouteDuration]);
