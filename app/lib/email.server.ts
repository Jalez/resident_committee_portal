 ...
 */
export async function parseReimbursementReply(content: string): Promise<"approved" | "rejected" | "unclear"> {
    // Import from openrouter.server.ts
    const { isAIParsingEnabled, parseReplyWithAI, getKeywords } = await import("./openrouter.server");
    ...
    if (aiConfig.enabled && aiConfig.apiKey && aiConfig.model) {
        ...
        const aiDecision = await parseReplyWithAI(content, aiConfig.apiKey, aiConfig.model);
...
export async function sendReimbursementEmail(
    ...
    try {
        ...
        console.log(`[sendReimbursementEmail] Successfully sent email for: ${data.itemName}, messageId: ${responseData?.id}`);
        return {
            success: true,
