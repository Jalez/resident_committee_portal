...
import type { MinuteStorageAdapter, UploadOptions, UploadResult } from "./types";
...
export class FilesystemMinuteStorage implements MinuteStorageAdapter {
    ...
    async deleteFile(pathname: string): Promise<void> {
        ...
        try {
            ...
            console.error(`[FilesystemMinuteStorage] Failed to delete ${fullPath}`, error);
        }
    }
    ...
    async getMinuteContentBase64(fileUrl: string): Promise<string | null> {
        ...
        try {
            ...
            console.error(`[FilesystemMinuteStorage] Failed to read ${fullPath}`, error);
            return null;
        }
    }
    ...
    async listMinutes(): Promise<{ url: string; pathname: string }[]> {
        ...
        try {
            ...
            const entries = await fs.readdir(fullPrefixPath, { recursive: true, withFileTypes: true });

            return entries
                .filter(e => e.isFile())
                .map(e => {
                    const relativePath = path.relative(this.uploadDir, path.join(e.parentPath, e.name));
