...
export function getEnvStatus(): EnvStatus {
	const categories: EnvCategory[] = [
		...
		},
		{
			...
			nameFi: "Kuittien tallennus",
			description: "Storage provider for receipts",
			descriptionFi: "Kuitteja varten käytettävä tallennus",
			icon: "cloud_upload",
			variables: [
				{
					name: "RECEIPT_STORAGE_PROVIDER",
					description: "Receipt storage provider",
					descriptionFi: "Kuittien tallennuspalvelu",
					required: true,
					isSet: isSet("RECEIPT_STORAGE_PROVIDER"),
					helpText: "Options: 'vercel-blob', 'filesystem', or 's3'",
				},
				{
				...
				{
					name: "S3_ENDPOINT",
					description: "S3 endpoint URL (required if provider is 's3')",
					descriptionFi: "S3-päätepisteen URL (tarvitaan jos palvelu on 's3')",
					required: false,
					isSet: isSet("S3_ENDPOINT"),
					helpText: "Leave empty for AWS S3, or set for S3-compatible services (e.g., MinIO, DigitalOcean Spaces)",
				},
				{
					name: "S3_REGION",
					description: "S3 region (required if provider is 's3')",
					descriptionFi: "S3-alue (tarvitaan jos palvelu on 's3')",
					required: false,
					isSet: isSet("S3_REGION"),
					helpText: "Default: us-east-1",
				},
				{
					name: "S3_BUCKET",
					description: "S3 bucket name (required if provider is 's3')",
					descriptionFi: "S3-äkillinen nimi (tarvitaan jos palvelu on 's3')",
					required: false,
					isSet: isSet("S3_BUCKET"),
				},
				{
					name: "S3_ACCESS_KEY_ID",
					description: "S3 access key ID (required if provider is 's3')",
					descriptionFi: "S3-käyttöavaimen tunnus (tarvitaan jos palvelu on 's3')",
					required: false,
					isSet: isSet("S3_ACCESS_KEY_ID"),
				},
				{
					name: "S3_SECRET_ACCESS_KEY",
					description: "S3 secret access key (required if provider is 's3')",
					descriptionFi: "S3-salainen käyttöavain (tarvitaan jos palvelu on 's3')",
					required: false,
					isSet: isSet("S3_SECRET_ACCESS_KEY"),
				},
				{
					name: "S3_PUBLIC_URL",
					description: "Public URL for S3 bucket (optional, for 's3' provider)",
					descriptionFi: "Julkinen URL S3-äkilliselle (valinnainen, 's3'-palvelulle)",
					required: false,
					isSet: isSet("S3_PUBLIC_URL"),
					helpText: "CDN or public URL for accessing files. If not set, signed URLs will be generated.",
				},
				{
					name: "S3_FORCE_PATH_STYLE",
					description: "Force path-style URLs for S3 (optional, for 's3' provider)",
					descriptionFi: "Pakota polkutyyliset URL:t S3:lle (valinnainen, 's3'-palvelulle)",
					required: false,
					isSet: isSet("S3_FORCE_PATH_STYLE"),
					helpText: "Set to 'true' for S3-compatible services like MinIO that require path-style URLs",
				},
			],
			isFullyConfigured: false,
			configuredCount: 0,
