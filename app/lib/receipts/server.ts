...
function receiptToFile(receipt: {
    pathname: string;
    name: string | null;
    url: string;
    createdAt: Date;
...
export function createReceiptStorage(
    ...
    switch (config.provider) {
        ...
        default:
            throw new Error(
                `Unsupported receipt storage provider: ${config.provider}`,
            );
    }
}
...
export async function getUnconnectedReceiptsByYear(): Promise<
    ReceiptsByYear[]
> {
...
export async function getReceiptsForPurchaseEdit(
    ...
    const merged = allReceipts.filter(
        (r) =>
            linkedReceiptIds.has(r.id) ||
            !Array.from(linkedReceiptIds).some(
                (id) => allReceipts.find((ar) => ar.id === id)?.id === r.id,
            ),
    );
