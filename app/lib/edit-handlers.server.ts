...
export async function createEditLoader<
	...
	if (!returnUrl) {
		...
		if (referer) {
			...
			if (
				...
				!refererUrl.pathname.includes("/edit")
			) {
...
>({
	...
	if (beforeUpdate) {
		const error = await beforeUpdate({
			db,
			entity,
			formData,
...
export async function createEmailAction<T extends { id: string }>({
	...
	try {
		...
		if (result.success) {
			...
			if (successRedirect) {
				return redirectWithStatus(successRedirect(entity), "success", "sent");
			}
			return { success: true };
		} else {
