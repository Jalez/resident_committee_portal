...
export interface RelationshipContext {
    ...
    valueSource: "manual" | "receipt" | "reimbursement" | "transaction" | "budget" | "unknown";
...
async function fetchRelationshipGraph(
    ...
    // Deduplicate (naive)
    graph.receipts = [...new Map(graph.receipts.map(item => [item.id, item])).values()];
    graph.reimbursements = [...new Map(graph.reimbursements.map(item => [item.id, item])).values()];
    graph.transactions = [...new Map(graph.transactions.map(item => [item.id, item])).values()];
...
async function buildContextFromSource(
    ...
    // Populate linked IDs
    graph.receipts.forEach(r => context.linkedEntityIds.push(`receipt:${r.id}`));
    graph.reimbursements.forEach(p => context.linkedEntityIds.push(`reimbursement:${p.id}`));
    graph.transactions.forEach(t => context.linkedEntityIds.push(`transaction:${t.id}`));
    ...
    if (source.type === "receipt") {
        ...
        context.totalAmount = content?.totalAmount ? Number(content.totalAmount) : null;
        ...
        try {
            if (type === "transaction") {
                ...
                await db.updateTransaction(id, {
                    ...
                    amount: context.totalAmount ? (-context.totalAmount).toString() : undefined, // Transactions are usually negative expenses
                ...
                await db.updatePurchase(id, {
                    amount: context.totalAmount ? context.totalAmount.toString() : undefined,
                    description: context.description || undefined,
