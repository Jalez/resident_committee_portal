...
import type { RelationshipEntityType, NewEntityRelationship } from "~/db/schema";
import { SETTINGS_KEYS } from "../openrouter.server";
import { getEntityTitle } from "../entity-converters";
...
async function createDraftEntity(
  ...
  try {
    ...
    switch (suggestion.entityType) {
      case "transaction": {
        const transaction = await db.createTransaction({
          ...
          date: suggestion.data.date ? new Date(suggestion.data.date as string) : new Date(),
          status: "draft",
          createdBy,
        });
      ...
      default:
        throw new Error(`Unsupported entity type for draft creation: ${suggestion.entityType}`);
    }
    ...
    console.error(`[Analyzer] Failed to create draft ${suggestion.entityType}:`, error);
    throw error;
  }
}
...
- Confidence < 0.7 means skip the suggestion`;
    ...
    for (const suggestion of suggestions) {
      try {
        const created = await createDraftEntity(db, "receipt", receiptId, suggestion, createdBy);
        if (created) {
          result.created.push(created);
        }
      } catch (error) {
        result.errors.push(`Failed to create ${suggestion.entityType}: ${error}`);
      }
    }
...
export async function analyzeReimbursement(
  ...
  try {
    ...
    const relationships = await db.getEntityRelationships("reimbursement", reimbursementId);
    ...
    const receipts = receiptIds.length > 0 
      ? await Promise.all(receiptIds.map(id => db.getReceiptById(id))).then(rs => rs.filter((r): r is NonNullable<typeof r> => r !== null))
      : [];
        ...
        if (content?.items) {
          ...
          for (const item of items) {
            ...
            if (price >= 20) {
              ...
              const durableKeywords = ['tool', 'equipment', 'electronic', 'cable', 'adapter', 'device', 'monitor', 'keyboard', 'mouse', 'chair', 'desk'];
              ...
              if (isDurable) {
                const suggestion: SuggestedEntity = {
                  ...
                  data: {
                    ...
                    purchasedAt: content.purchaseDate?.toISOString() || new Date().toISOString(),
                ...
                try {
                  const created = await createDraftEntity(db, "reimbursement", reimbursementId, suggestion, createdBy);
                  if (created) {
                    result.created.push(created);
                  }
                } catch (error) {
                  result.errors.push(`Failed to create inventory item: ${error}`);
                }
              }
            }
          }
    ...
    const existingRelationships = await db.getEntityRelationships("reimbursement", reimbursementId);
    const hasTransaction = existingRelationships.some(r => r.relationBType === "transaction" || r.relationAType === "transaction");
    if (!hasTransaction) {
      const transactionSuggestion: SuggestedEntity = {
        ...
        data: {
          ...
          date: reimbursement.createdAt?.toISOString() || new Date().toISOString(),
          category: "other",
        },
        confidence: 0.95,
        reasoning: "Every reimbursement needs a corresponding expense transaction",
      };

      try {
        const created = await createDraftEntity(db, "reimbursement", reimbursementId, transactionSuggestion, createdBy);
        if (created) {
          result.created.push(created);
        }
      } catch (error) {
...
Link only if the transaction description/category clearly matches the budget purpose.`;
      ...
      if (text) {
        try {
          const parsed = JSON.parse(text);
          if (parsed.shouldLink && parsed.budgetId && parsed.confidence >= 0.7) {
...
export async function analyzeMinute(
  ...
  try {
    ...
    if (minute.title && minute.title.length > 0) {
      ...
      try {
        const created = await createDraftEntity(db, "minute", minuteId, newsSuggestion, createdBy);
        if (created) {
          result.created.push(created);
        }
      } catch (error) {
